    ...
    @if($berita->count() > 0)
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
            @foreach($berita as $item)
                <article class="space-y-4 group cursor-pointer" onclick="window.location.href='{{ route('student.berita.detail', $item->id) }}'">
                    <div class="aspect-[16/10] overflow-hidden rounded-2xl bg-slate-200 shadow-sm">
                        @if($item->image)
                            <img src="{{ $item->image }}" class="w-full h-full object-cover group-hover:scale-105 transition" alt="{{ $item->title }}" />
                        @else
                    ...
                    <span class="text-blue-600 font-bold text-xs uppercase tracking-widest">{{ $item->category ?? 'Berita' }}</span>
                    <h3 class="text-xl font-bold text-slate-800 leading-snug group-hover:text-blue-600 transition">{{ $item->title }}</h3>
                    <p class="text-slate-500 text-sm leading-relaxed">{{ $item->excerpt ?? substr(strip_tags($item->content), 0, 100) }}...</p>
