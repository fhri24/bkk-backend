...
@if(session('success'))
<div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl text-green-700 flex items-center gap-2 shadow-sm animate-fade-in">
    <i class="fas fa-check-circle text-lg"></i> 
    <span class="font-medium">{{ session('success') }}</span>
</div>
@elseif(session('error'))
<div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl text-red-700 flex items-center gap-2 shadow-sm animate-fade-in">
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
    ...
    <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-md font-medium">Total Grup: {{ $summaryByMajor->count() }}</span>
        </div>
        <div class="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
            @forelse($summaryByMajor as $sm)
            <div class="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg border border-slate-100 hover:border-slate-200 transition">
                <div class="flex items-center gap-3">
                    <span class="font-semibold text-slate-800 text-sm bg-white border border-slate-200 w-10 h-8 flex items-center justify-center rounded-md shadow-sm">{{ $sm->major }}</span>
                ...
                <form action="{{ route('admin.students.destroy.by.major') }}" method="POST" 
                    ...
                    <button type="submit" class="text-xs text-red-500 hover:text-red-700 hover:bg-red-50 px-2 py-1 rounded transition flex items-center gap-1">
    ...
    <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-md font-medium">Total Angkatan: {{ $summaryByYear->count() }}</span>
        </div>
        <div class="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
            ...
            <div class="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg border border-slate-100 hover:border-slate-200 transition">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-medium bg-green-50 text-green-700 border border-green-100 px-2 py-0.5 rounded-full">{{ $sy->total }} Alumni</span>
                </div>
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
    ...
    <div class="p-6 border-b border-slate-200 flex flex-wrap items-center justify-between gap-4 bg-slate-50/50">
        <h3 class="text-lg font-bold text-slate-800 flex items-center">
            <i class="fas fa-users text-green-600 mr-3 bg-green-50 p-2.5 rounded-lg border border-green-100"></i> Daftar Utama Alumni
        </h3>
        
        <div class="flex items-center gap-3 flex-wrap">
            {{-- Filter Form Terpadu (Mendukung filter gabungan Tahun + Jurusan) --}}
            <form method="GET" action="{{ route('admin.students.index') }}" class="flex items-center gap-2 flex-wrap md:flex-nowrap">
                {{-- Filter Angkatan --}}
                ...
                @if(request('year') || request('major'))
                    <a href="{{ route('admin.students.index') }}" class="text-xs text-slate-500 hover:text-red-500 underline font-medium px-1">
                        Reset Filter
                    </a>
                @endif
            </form>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/80 transition">
                    ...
                    <td class="px-5 py-3.5 text-center">
                        @if($student->gender === 'L')
                            <span class="px-2.5 py-0.5 text-xs font-bold rounded-md bg-blue-50 text-blue-600 border border-blue-100">L</span>
                        @elseif($student->gender === 'P')
                            <span class="px-2.5 py-0.5 text-xs font-bold rounded-md bg-pink-50 text-pink-600 border border-pink-100">P</span>
                        @else
                    ...
                    <td class="px-5 py-3.5">
                        <span class="px-2.5 py-0.5 text-xs rounded-full bg-green-50 text-green-700 font-semibold border border-green-100">
                    ...
                    <td class="px-5 py-3.5">
                        <div class="flex items-center justify-center gap-3">
                            ...
                            <form action="{{ route('admin.students.destroy', $student->student_id) }}" method="POST"
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="inline-flex items-center gap-1 text-xs text-red-500 hover:text-red-700 font-semibold hover:underline">
                    ...
                    <td colspan="8" class="text-center text-slate-400 py-16 bg-slate-50/30">
                        ...
                        @if(request('year') || request('major'))
                            <span class="block text-xs text-slate-400 mt-1">
                                ...
                                @if(request('year')) <strong>[Angkatan {{ request('year') }}]</strong> @endif
                                @if(request('major')) <strong>[Jurusan {{ request('major') }}]</strong> @endif
                            </span>
                        @endif
                    </td>
                </tr>
...
<div id="modalImport"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden transform transition-transform border border-slate-100">
