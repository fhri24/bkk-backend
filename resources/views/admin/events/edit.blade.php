        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
            ...
            <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2 items-center">
                            <input type="date" name="start_date_day" value="{{ old('start_date_day', $event->start_date->format('Y-m-d')) }}"
                                class="flex-1 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none" required>
                            <input type="time" name="start_date_time" value="{{ old('start_date_time', $event->start_date->format('H:i')) }}"
                    ...
                    <div>
                        ...
                        <div class="flex gap-2 items-center">
                            <input type="date" name="end_date_day" value="{{ old('end_date_day', $event->end_date->format('Y-m-d')) }}"
                                class="flex-1 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none" required>
                            <input type="time" name="end_date_time" value="{{ old('end_date_time', $event->end_date->format('H:i')) }}"
                                class="w-32 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none" required>
                            <span class="text-sm font-bold text-slate-600">WIB</span>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="5"
                            class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none">{{ old('description', $event->description) }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2 bg-blue-50 border border-blue-200 p-4 rounded-lg">
                        ...
                        <p class="text-sm text-slate-600 ml-8 mt-1">Jika tidak dicentang, acara akan tersimpan sebagai draft.</p>
                    </div>
                ...
                <div class="flex justify-end gap-3 border-t pt-6 mt-6">
                    ...
                    <button type="submit"
                        class="px-6 py-2.5 bg-amber-500 text-white font-bold rounded-lg hover:bg-amber-600 transition">
