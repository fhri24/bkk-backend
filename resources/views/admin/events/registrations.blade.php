                ...
                <form method="GET" class="flex items-end gap-4 flex-wrap">
                        ...
                        <select name="event_slug"
                            ...
                            @foreach ($events as $event)
                                <option value="{{ $event->id }}"
                                    {{ request('event_slug') == $event->id ? 'selected' : '' }}>
                                    {{ $event->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex-1 min-w-xs">
                        ...
                        <select name="status"
                            ...
                            <option value="registered" {{ request('status') == 'registered' ? 'selected' : '' }}>Terdaftar
                            </option>
                    ...
                    <div class="flex gap-3 items-center mt-4 w-full flex-wrap">
                        <a href="{{ route('admin.event-registrations.export.print', request()->only(['event_slug', 'status'])) }}"
                            target="_blank"
                            ...
                            @forelse($registrations as $reg)
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="px-6 py-4 text-sm text-slate-600">
                                        {{ $reg->registered_at ? $reg->registered_at->format('d M Y') : '-' }}</td>
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex justify-center gap-2">
                                            <button
                                                onclick="openEditModal({{ $reg->event_registration_id }}, '{{ $reg->status }}', '{{ addslashes($reg->admin_notes ?? '') }}')"
                                                class="text-blue-600 hover:text-blue-800 font-bold" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <form
                                                action="{{ route('admin.event-registrations.destroy', $reg->event_registration_id) }}"
                                                method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Yakin ingin menghapus?')"
                                                    class="text-red-600 hover:text-red-800 font-bold" title="Hapus">
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 text-white flex justify-between items-center">
                ...
                <button onclick="closeEditModal()"
                    class="text-white text-2xl font-bold hover:text-blue-200">&times;</button>
            </div>
            <form id="editForm" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="admin_notes" id="adminNotesInput" rows="3"
                        class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="closeEditModal()"
                        class="flex-1 px-4 py-2 border border-slate-300 text-slate-700 rounded-lg font-medium hover:bg-slate-50 transition">Batal</button>
                    <button type="submit"
                        class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition">Simpan</button>
                </div>
            </form>
        </div>
    </div>
