        ...
        <form method="POST" action="{{ route('admin.tips.update', $tip) }}" class="space-y-6">
            ...
            <div class="table-custom p-8 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $tip->judul) }}" class="form-input"
                    ...
                    <div>
                        ...
                        <select name="kategori" class="form-input" required onchange="updateIcon(this.value)">
                            ...
                            @foreach ($kategoriList as $kat)
                                <option value="{{ $kat }}"
                                    {{ old('kategori', $tip->kategori) === $kat ? 'selected' : '' }}>{{ $kat }}
                                </option>
                            @endforeach
                        </select>
                        @error('kategori')
                    ...
                    <div>
                        ...
                        <div class="flex gap-3 items-center">
                            <div
                                ...
                                <i id="iconEl" class="{{ old('icon', $tip->icon ?? 'fas fa-lightbulb') }}"></i>
                            </div>
                            <input type="text" name="icon" id="iconInput"
                                value="{{ old('icon', $tip->icon ?? 'fas fa-lightbulb') }}" class="form-input flex-1"
                ...
                <div>
                    ...
                    <textarea name="ringkasan" class="form-input" rows="3" maxlength="500" required
                        placeholder="Deskripsi singkat yang tampil di card">{{ old('ringkasan', $tip->ringkasan) }}</textarea>
                    @error('ringkasan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400 mb-2">
                        Bisa pakai tag HTML sederhana: &lt;h2&gt;, &lt;h3&gt;, &lt;p&gt;, &lt;ul&gt;, &lt;li&gt;,
                        &lt;strong&gt;
                    </p>
                    <textarea name="konten" class="form-input form-textarea" required placeholder="Tulis konten lengkap tips di sini...">{{ old('konten', $tip->konten) }}</textarea>
                    @error('konten')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid sm:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="number" name="urutan" min="0" value="{{ old('urutan', $tip->urutan ?? 0) }}"
                            class="form-input">
