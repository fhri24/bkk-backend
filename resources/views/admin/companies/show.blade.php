    ...
    <div class="p-6 max-w-5xl">
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('admin.companies.edit', $company->company_id) }}"
                                ...
                                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                        <div>
                                            ...
                                            <p class="text-sm text-slate-600">{{ ucfirst($job->visibility) }} •
                                                {{ $job->job_type }}</p>
                                        </div>
                                        <div class="flex gap-2">
                                            <a href="{{ route('admin.jobs.edit', $job->job_id) }}"
                                                class="inline-flex items-center rounded-lg bg-blue-600 px-3 py-2 text-white text-sm hover:bg-blue-700 transition">Edit</a>
                                            <a href="{{ route('admin.jobs.edit', $job->job_id) }}"
                                                class="inline-flex items-center rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700 hover:bg-slate-200 transition">Edit</a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
