    ...
    <div class="p-6 max-w-4xl">
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('admin.jobs.index') }}"
                class="inline-flex items-center rounded-lg bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200 transition">
        ...
        <form action="{{ route('admin.jobs.update', $job->job_id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid gap-6">
                ...
                <div>
                    ...
                    <select name="company_id" id="company_id"
                        class="mt-2 block w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100"
                        required>
                        <option value="">-- Pilih Perusahaan --</option>
                        @foreach ($companies as $company)
                            <option value="{{ $company->company_id }}"
                                {{ old('company_id', $job->company_id) == $company->company_id ? 'selected' : '' }}>
                    ...
                    <select name="major_id" id="major_id"
                        ...
                        @foreach ($majors as $major)
                            <option value="{{ $major->id }}"
                                {{ old('major_id', $job->major_id) == $major->id ? 'selected' : '' }}>
                                {{ $major->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $job->title) }}"
                        placeholder="Contoh: Developer PHP"
                ...
                {{-- Poster / Banner --}}
                <div>
                    <label for="image" class="block text-sm font-medium text-slate-700">Poster / Banner Lowongan <span
                ...
                <div>
                    ...
                    <textarea name="responsibilities" id="responsibilities" rows="4"
                        placeholder="Daftar tanggung jawab pekerjaan (pisahkan tiap baris)..."
                ...
                <div>
                    <label for="skill_required" class="block text-sm font-medium text-slate-700">Skill yang Dibutuhkan <span
                            class="text-slate-400">(Opsional)</span></label>
                    <input type="text" name="skill_required" id="skill_required"
                        value="{{ old('skill_required', $job->skill_required) }}" placeholder="Contoh: PHP, Laravel, MySQL"
                        class="mt-2 w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
                </div>
                ...
                <div class="grid gap-4 lg:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="salary" id="salary" value="{{ old('salary', $job->salary) }}"
                            placeholder="Contoh: Kompetitif / Rp 5.000.000"
                            class="mt-2 w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
                    </div>
                </div>

                {{-- Jenis Pekerjaan & Visibility --}}
                <div class="grid gap-4 lg:grid-cols-2">
                    <div>
                        ...
                        <select name="job_type" id="job_type"
                            ...
                            @foreach (['Full-time', 'Part-time', 'Contract', 'Internship'] as $type)
                                <option value="{{ $type }}"
                                    {{ old('job_type', $job->job_type) == $type ? 'selected' : '' }}>{{ $type }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="visibility" id="visibility"
                            ...
                            @foreach (['public' => 'Public', 'alumni_only' => 'Alumni Only', 'private' => 'Private', 'internal' => 'Internal'] as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('visibility', $job->visibility) === $value ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div>
                    ...
                    <input type="date" name="expired_at" id="expired_at"
                        value="{{ old('expired_at', $job->expired_at ? $job->expired_at->format('Y-m-d') : '') }}"
                        class="mt-2 w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100"
                        required>
                </div>
                ...
                <div class="flex flex-wrap gap-3 pt-2">
                    ...
                    <a href="{{ route('admin.jobs.index') }}"
                        class="inline-flex items-center rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition">Batal</a>
                </div>
