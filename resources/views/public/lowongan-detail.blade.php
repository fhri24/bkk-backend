...
@section('content')
    ...
    @php
        ...
        // Cek apakah user sudah menyimpan lowongan ini (Opsional untuk inisialisasi tombol)
        $isSaved = false;
        if (auth()->check()) {
            ...
            <div style="display:flex; gap:32px; align-items:flex-start; flex-wrap:wrap;">
                ...
                <div style="flex:1; min-width:0;">
                    ...
                    <div style="display:flex; flex-wrap:wrap; gap:12px;">
                        <span
                            style="background:rgba(255,255,255,.15); padding:8px 16px; border-radius:8px; display:flex; align-items:center; gap:8px; font-size:14px; font-weight:600; backdrop-filter:blur(4px);">
                ...
                <div class="sidebar-sticky">
                    ...
                    <div class="sidebar-card">
                        ...
                        @auth
                            ...
                            <button class="btn-warning" id="saveBtn" onclick="toggleSaveJob(this, {{ $job->job_id }})"
                                style="background: {{ $isSaved ? '#dc2626' : '#fbbf24' }}; color: {{ $isSaved ? '#fff' : '#1e293b' }};">
                                <i class="fas fa-bookmark {{ $isSaved ? 'fa-solid' : 'fa-regular' }}" id="saveBtnIcon"
                                    style="margin-right:8px;"></i>
                                <span id="saveBtnText">{{ $isSaved ? 'Tersimpan' : 'Simpan Lowongan' }}</span>
                            </button>
                        @else
                            <a href="{{ Route::has('login') ? route('login') : '/login' }}" class="btn-primary"
                        ...
                        <div class="sidebar-card">
                            ...
                            @foreach ($similarJobs as $sim)
                                <div class="similar-item">
                                    ...
                                    <a
                                        href="{{ Route::has('public.lowongan.detail') ? route('public.lowongan.detail', $sim->job_id) : url('/lowongan/' . $sim->job_id) }}">
    ...
    <div id="applicationModal">
        <div class="modal-box">
            <div
                style="background:linear-gradient(135deg,#2563eb,#1e40af); padding:28px 32px; border-radius:20px 20px 0 0; display:flex; justify-content:space-between; align-items:center;">
                <h2
                    style="color:#fff; font-size:20px; font-weight:900; margin:0; display:flex; align-items:center; gap:10px;">
            ...
            <form id="applicationForm" action="{{ route('universal.apply', $job->job_id) }}" method="POST"
                enctype="multipart/form-data" style="padding:32px; display:flex; flex-direction:column; gap:20px;">
                @csrf
                <input type="hidden" name="job_id" value="{{ $job->job_id }}">

                <div>
                    <label class="form-label">Nama Lengkap</label>
                    <input type="text" class="form-input bg-slate-50" value="{{ auth()->user()->name ?? '' }}"
                        readonly>
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="4" class="form-input" placeholder="Tulis pesan singkat untuk perusahaan..."></textarea>
                </div>
    ...
    <script>
        // FIX: Function toggleSaveJob dengan fetch dan parameter jobId otomatis
        function toggleSaveJob() {
            ...
            // Mencegah klik ganda saat proses
            btn.disabled = true;
        ...
        document.getElementById('applicationForm').addEventListener('submit', function(e) {
            ...
            fetch(form.action, {
                ...
                .then(async response => {
                    ...
                    if (!response.ok) {
                        throw new Error(data ? data.message : 'Terjadi kesalahan sistem (Error ' + response
                            .status + ')');
                    }
