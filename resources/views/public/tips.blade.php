...
<style>
  ...
  .prose-content h2 { font-size:1.25rem; font-weight:700; margin:1.25rem 0 0.5rem; color:#1e293b; }
  .prose-content h3 { font-size:1.1rem; font-weight:600; margin:1rem 0 0.4rem; color:#334155; }
  .prose-content p  { margin-bottom:0.75rem; color:#475569; line-height:1.7; }
  .prose-content ul { list-style:disc; padding-left:1.5rem; margin-bottom:0.75rem; color:#475569; }
  .prose-content ol { list-style:decimal; padding-left:1.5rem; margin-bottom:0.75rem; color:#475569; }
  .prose-content li { margin-bottom:0.3rem; }
  .prose-content strong { color:#1e293b; }
  .tip-modal { display:none; position:fixed; z-index:1000; inset:0; background:rgba(0,0,0,0.5); animation:fadeIn 0.2s ease; }
  .tip-modal.show { display:flex; align-items:center; justify-content:center; }
  .tip-modal-content { background:white; border-radius:24px; width:90%; max-width:720px; max-height:88vh; overflow-y:auto; animation:slideUp 0.25s ease; }
...
@section('content')
  ...
  <div class="bg-slate-900 relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 py-20 relative z-10 text-center text-white">
      ...
      <p class="text-lg opacity-80 max-w-2xl mx-auto leading-relaxed mb-6">
      ...
      @guest
        <a href="{{ route('public.tutorial') }}"
           class="inline-flex items-center gap-2 text-sm text-blue-200 hover:text-white transition">
  ...
  <div class="page-transition container mx-auto px-6 py-16">
    ...
    <div class="mb-14">
      ...
      <div class="grid md:grid-cols-3 gap-6">
        ...
        <div class="tip-card bg-gradient-to-br from-[#001f3f] to-[#003d6b] text-white rounded-[28px] p-8 cursor-pointer"
          ...
          <span class="text-[10px] font-bold uppercase tracking-widest text-blue-300 mb-2 block">{{ $item->kategori }}</span>
    ...
    <div class="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between mb-8">
      ...
      <form method="GET" action="{{ route('public.tips') }}" class="flex gap-2">
        ...
        @if(request()->hasAny(['search','kategori']))
          <a href="{{ route('public.tips') }}" class="bg-slate-100 text-slate-600 px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-slate-200 transition">
            Reset
          </a>
        @endif
      </form>
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
      ...
      @php
        $colorMap = [
          'Interview'       => ['bg'=>'bg-blue-50',   'text'=>'text-blue-600',   'badge'=>'bg-blue-50 text-blue-600'],
          'Psikotes'        => ['bg'=>'bg-purple-50',  'text'=>'text-purple-600', 'badge'=>'bg-purple-50 text-purple-600'],
          'CV & Portofolio' => ['bg'=>'bg-orange-50',  'text'=>'text-orange-600', 'badge'=>'bg-orange-50 text-orange-600'],
          'Dunia Kerja'     => ['bg'=>'bg-green-50',   'text'=>'text-green-600',  'badge'=>'bg-green-50 text-green-600'],
          'Wirausaha'       => ['bg'=>'bg-amber-50',   'text'=>'text-amber-600',  'badge'=>'bg-amber-50 text-amber-600'],
          'Lainnya'         => ['bg'=>'bg-slate-50',   'text'=>'text-slate-600',  'badge'=>'bg-slate-100 text-slate-500'],
        ];
      ...
      <div class="tip-card bg-white border border-slate-100 rounded-[24px] p-7 shadow-sm cursor-pointer"
        ...
        <div class="flex items-start justify-between mb-5">
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-xl {{ $color['bg'] }} {{ $color['text'] }}">
            <i class="{{ $tip->icon }}"></i>
          </div>
          <span class="text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full {{ $color['badge'] }}">
            {{ $tip->kategori }}
          </span>
        </div>
  ...
  <div class="tip-modal-content">
    <div class="sticky top-0 bg-white border-b border-slate-100 px-8 py-5 flex items-center justify-between rounded-t-3xl z-10">
      <div class="flex items-center gap-3">
        <div id="modalIcon" class="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center text-lg"></div>
        <div>
      ...
      <button onclick="closeTip()" class="w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 transition flex items-center justify-center text-slate-500 text-lg">&times;</button>
    </div>
    <div class="px-8 py-7">
