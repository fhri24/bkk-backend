...
@section('content')
    ...
    <div class="bg-slate-900 relative overflow-hidden">
        ...
        <div class="container mx-auto px-6 py-20 relative z-10 text-center text-white">
            ...
            <div
                class="flex flex-col md:flex-row justify-center gap-3 max-w-2xl mx-auto bg-white/10 p-2 rounded-2xl backdrop-blur-md">
    ...
    <div class="flex flex-col items-center justify-center mb-12 gap-4 text-center">
        <div class="min-w-0">
            <span
                class="inline-block px-3 py-1 bg-blue-100 text-blue-600 text-[10px] font-black uppercase tracking-widest rounded-md mb-2">Job
                Board</span>
            <h2 class="text-4xl font-extrabold text-[#001f3f]">Lowongan Terbaru</h2>
        </div>

        {{-- Filter Visibility --}}
        <div id="visGroup" class="flex flex-wrap justify-center items-center bg-white p-1.5 rounded-2xl shadow-sm border border-slate-200 gap-1">
            <button class="vis-btn px-4 py-2.5 rounded-xl text-sm font-bold text-slate-500 transition"
                data-v="all">Semua</button>
            <button class="vis-btn px-4 py-2.5 rounded-xl text-sm font-bold text-slate-500 transition"
                data-v="public">Public</button>
            <button class="vis-btn px-4 py-2.5 rounded-xl text-sm font-bold text-slate-500 transition"
                data-v="alumni_only">Alumni Only</button>
            <button class="vis-btn px-4 py-2.5 rounded-xl text-sm font-bold text-slate-500 transition"
                data-v="private">Private</button>
            <button class="vis-btn px-4 py-2.5 rounded-xl text-sm font-bold text-slate-500 transition"
                data-v="internal">Internal</button>
        </div>
    </div>
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 sticky top-24">
                    ...
                    <div id="typeGroup" class="space-y-2 mb-8">
                        @foreach (['Full-time', 'Part-time', 'Contract', 'Internship'] as $type)
                            <button
                                class="type-btn w-full text-left px-4 py-3 rounded-xl border border-slate-100 text-sm font-semibold transition flex justify-between items-center"
                                data-t="{{ $type }}">
                                <span>{{ $type }}</span><i
                                    class="fas fa-check-circle text-blue-500 opacity-0 transition-opacity"></i>
                            </button>
                        @endforeach
                    </div>
                    ...
                    <div id="majorGroup" class="space-y-2 mb-8">
                        <button
                            class="major-btn w-full text-left px-4 py-3 rounded-xl border border-slate-100 text-sm font-semibold transition"
                            data-m="all">Semua Jurusan</button>
                    ...
                    <button id="btnReset"
                        class="w-full bg-slate-50 hover:bg-slate-100 text-slate-500 py-3 rounded-xl text-xs font-bold transition">
                ...
                <div id="jobList" class="space-y-6">
                    @forelse($jobs as $job)
                        ...
                        <div class="jcard bg-white p-7 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6 hover:shadow-xl transition"
                            data-title="{{ strtolower($job->title) }}" data-company="{{ strtolower($cName) }}"
                            data-type="{{ $jType }}" data-vis="{{ $jVis }}" data-major="{{ $jMajor }}">
                            {{-- SINKRON DENGAN MAJOR ID --}}

                            <div
                                class="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center shrink-0 border border-slate-100 overflow-hidden">
                            ...
                            <div class="flex-1">
                                <div class="flex flex-wrap items-start justify-between gap-2 mb-1">
                                    ...
                                    <span
                                        class="{{ $badge }} text-[9px] font-black px-3 py-1 rounded-full uppercase">{{ $badgeTxt }}</span>
                                </div>
                                <p class="text-blue-600 font-bold text-sm mb-4">{{ $cName }}</p>

                                <div class="flex flex-wrap gap-5 text-xs text-slate-500 font-bold mb-5">
                                    <span><i
                                            class="fas fa-map-marker-alt text-blue-400 mr-1"></i>{{ $jLoc }}</span>
                                    ...
                                    <span class="text-blue-600"><i
                                            class="fas fa-graduation-cap text-blue-400 mr-1"></i>{{ $job->major->name ?? 'Semua Jurusan' }}</span>
                                </div>
    ...
    <script>
        function toggleSaveJob(btn, jobId) {
            fetch('/lowongan/' + jobId + '/save-toggle', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    ...
                    if (data.saved) {
                        btn.classList.remove('bg-gray-50', 'text-gray-400', 'border-slate-200');
