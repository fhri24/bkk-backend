...
<body class="bg-slate-50 text-slate-900">
        ...
        <div id="modal-tersimpan"
            ...
            <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-zoom-in">
                ...
                <div id="modal-saved-list" class="max-h-[60vh] overflow-y-auto p-4 space-y-3 hide-scrollbar">
                    @php
                        $saved_jobs = \App\Models\SavedJob::where('user_id', auth()->id())
                            ->with('job.company')
                            ->get();
                    @endphp
                    ...
                    @forelse($saved_jobs as $saved)
                        @if ($saved->job)
                            <div id="modal-item-{{ $saved->job_id }}"
                                class="group flex items-center justify-between p-4 hover:bg-slate-50 transition-all border-b border-slate-100 last:border-0">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button onclick="removeSavedJob({{ $saved->job_id }}, 'modal')"
                ...
                <div class="p-4 border-t border-slate-100 bg-slate-50 flex justify-center">
                    <a href="{{ route('universal.saved-jobs') }}" class="text-sm font-bold text-blue-600 hover:underline">
    ...
    <script>
        ...
        // 2. Fungsi Toggle Simpan (Menggunakan route universal)
            ...
            fetch(`/lowongan/${jobId}/save-toggle`, {
                ...
                .then(data => {
                    ...
                    if (data.status === 'added' || data.saved === true) {
                        ...
                        // Tombol detail (pakai inline style)
                        ...
                        // Update teks kalau ada
                        const txt = btn.querySelector('span');
                        if (txt) txt.textContent = 'Simpan Lowongan';
                    }
        ...
        // 3. Fungsi Hapus di Modal / Page (Menggunakan route universal)
